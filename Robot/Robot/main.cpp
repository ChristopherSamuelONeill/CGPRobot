...
const int SCREEN_HEIGHT = 768;		//height of window in pexils
...
void Game()
{
	...
	while (window.isOpen())
	{
		...
		if (clock.getElapsedTime().asSeconds()>REFRESH_RATE) // only draw and update objects on correct frequency
		{
			...
			if (g_level->m_iPlayerPicks >= g_level->m_iNumPicks)
			{
				//level completed
			...
			if (g_level->theRobot->m_fFuel <= 0) // if the player runsout of fuel
			{
				
				gameOver(false);
				window.close();
			}
			if (g_iLevel > g_iNumberOfLevels) // if the player has completed all the levels
			{
			...
			//draw robot
			drawRobot();
			//cout << "X : " << g_level->theRobot->getPos().x << "  Y : " << g_level->theRobot->getPos().y << "  X : " << g_level->theRobot->getPos().z << endl;
